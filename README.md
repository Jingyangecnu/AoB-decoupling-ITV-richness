...
table in the manuscript (a fixed random seed makes single-trait ITV values,
which rely on a Monte Carlo rarefaction step, identical across re-renders).

## Contents

- `Analysis_public.Rmd` — single, ordered R Markdown document that reads the
  public data files and produces all main-text figures/tables and
  ...
  - Table S1: Composition random-effect LRT decisions (Fig. 1 and Fig. 2 metrics)
  - Table S2: Aboveground vs belowground compartment interaction test
...
- `figures/` — main-text (Figure_1–5) and supplementary (Figure_S1–S3)
  figures, PDF (vector).
- `tables/` — Table S1 and Table S2, CSV.
